...
7. **Create incident** ボックスの下の **+** ボタンをクリックします。
    ...
    - 右側に **Make HTTP Request** フォームが表示されます。以下を設定します:
        - **Step Name**:
            ...
            ```
        :::note
        Step Name が表示されていない場合は右ペインタイトルの鉛筆アイコンをクリックして入力してください。
        :::
    ...
    ステップの設定画面は次のようになっているはずです:
    ![完成したステップ](/datadog-labs-ja/assets/dd-gcp-sre-fundamentals/step-completed.png)
...
8. ページ右上の **Save** をクリックし、次に **> Run** をクリックします。
    ...
    5. インシデントの内容を確認し、インシデントのステータスを **Active** から **Resolved** に変更します。
        :::note
        Get started の画面が出た場合は左ペインのIncident Response > Incident list から作成されたインシデントに直接アクセスします。また、Resolved にステータスを変更する際にAssign Incident Management Seat のポップアップが出た場合はそのままYes, Assign Seat を選択します。
        :::
