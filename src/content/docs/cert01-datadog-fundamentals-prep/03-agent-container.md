...
このラボでは、Docker 環境で Agent をインストールして設定します。Agent は、Storedog eコマース Web アプリケーションを構成する他の Docker コンテナとともに、専用のコンテナ内で実行されます。

前のラボでは、YAML 設定ファイルを使用して Agent を設定しました。このラボでは、環境変数とラベルを使用して Agent や他のコンテナを設定します。

**学習目標**:

* Docker 環境で Agent をインストールして設定する
* サービスコンテナにラベルを追加し、Agent がそれらを正しくタグ付けできるようにする
* Agent がメトリクスを正しく収集し、Datadog App 上で正しくタグ付けされていることを確認する

環境の準備ができたら、下にある **Start** ボタンをクリックしてください。

:::note
このラボは、10分間操作がないとタイムアウトします。
:::

このラボでは、各セクションが一連のステップに分割されています。セクション間を移動するには、各ヘッダーをクリックしてセクションを展開または折りたたんでください。

あなたのために用意された Datadog トレーニングアカウントの認証情報を使用して、[Datadog](https://app.datadoghq.com/account/login/) にログインしていることを確認してください。認証情報は、ラボのターミナルで `creds` を実行すると確認できます。
