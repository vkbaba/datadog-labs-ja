...
このラボでは、Datadog のインテグレーションについて学びます。本コースの前半で Agent について触れた際に説明したとおり、インテグレーションにはさまざまな種類があります。コアインテグレーションは Agent に同梱されており、最小限の設定で利用できるものです。

このラボでは、次のことを行います。

- コア Agent インテグレーションの仕組みを理解する
- Docker でインテグレーションを設定する
- Autodiscovery の仕組みを理解する

右下に表示される **Start** ボタンをクリックしてください。

---

このチャレンジでは、各セクションが一連のステップに分割されています。セクション間を移動するには、各ヘッダーをクリックしてセクションを展開または折りたたんでください。

割り当てられたアカウントで [Datadog](https://app.datadoghq.com/account/login/) にログインしていることを確認してください。アカウント情報は、左側のターミナルで `creds` を実行すると確認できます。
