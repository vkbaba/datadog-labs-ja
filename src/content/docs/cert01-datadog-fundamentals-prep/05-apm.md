...
このラボでは、Storedog のサービスが APM 向けにどのように計装されているかを確認し、その結果として得られるアプリケーショントレースを Datadog アプリで見ていきます。

**学習目標**:

* 言語固有のトレースライブラリを使ってトレースを収集する複数のサービスの構成を確認する
* Datadog で APM を探索する
* アプリケーショントレースとログの連携を確認する
* アプリケーションのプロファイリング構成を確認し、収集されたデータを観察する
